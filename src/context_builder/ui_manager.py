...
import os
from typing import List, Dict, Any, Callable, Optional, Union, TypedDict, cast


class PathSuggestion(TypedDict, total=False):
    """경로 제안을 위한 타입 정의"""
...
class UIManager:
    ...
    def __init__(self, core_controller: Any) -> None:
        """
        ...
        self.nl_input_var: Optional[tk.StringVar] = None
        self.watched_dirs_listbox: Optional[tk.Listbox] = None
        self.history_listbox: Optional[tk.Listbox] = None

    def display_main_window(self) -> None:
        """메인 애플리케이션 창을 표시합니다."""
        # tkinter 루트 윈도우 생성
        self.root = tk.Tk()
    ...
    def prompt_for_confirmation(
        self, action_description: str, options: List[Union[str, PathSuggestion]]
    ) -> Optional[Union[str, PathSuggestion]]:
        """
        ...
        result: List[Optional[Union[str, PathSuggestion]]] = [
            None
        ]  # 결과를 저장할 리스트 (비로컬 캡처를 위해)
    ...
    def display_results(self, message: str) -> None:
        """
        성공 또는 오류 메시지를 보여줍니다.
    ...
    def display_settings_dialog(
        ...
        # 설정 값을 담을 변수
        settings_vars: Dict[
            str, Dict[str, Union[tk.StringVar, tk.DoubleVar, tk.BooleanVar, tk.IntVar]]
        ] = {
            "llm": {
        ...
        ttk.Entry(
            llm_frame,
            textvariable=cast(tk.StringVar, settings_vars["llm"]["api_key"]),
            width=30,
            show="*",
        ...
        ttk.Combobox(
            llm_frame,
            textvariable=cast(tk.StringVar, settings_vars["llm"]["model_name"]),
        ...
        ttk.Scale(
            llm_frame,
            variable=cast(tk.DoubleVar, settings_vars["llm"]["temperature"]),
            from_=0.0,
            to=1.0,
        ...
        ttk.Combobox(
            ui_frame,
            textvariable=cast(tk.StringVar, settings_vars["ui"]["theme"]),
        ...
        ttk.Combobox(
            ui_frame,
            textvariable=cast(tk.StringVar, settings_vars["ui"]["language"]),
            values=["ko", "en"],
        ).grid(row=1, column=1, sticky=tk.W, padx=5)
        ...
        ttk.Checkbutton(
            ui_frame,
            variable=cast(tk.BooleanVar, settings_vars["ui"]["show_notifications"]),
        ).grid(row=2, column=1, sticky=tk.W, padx=5)
        ...
        ttk.Spinbox(
            ...
            to=100,
            textvariable=cast(
                tk.IntVar, settings_vars["file_operations"]["max_history"]
            ),
            width=5,
        ).grid(row=0, column=1, sticky=tk.W, padx=5)
        ...
        ttk.Checkbutton(
            ...
            variable=cast(
                tk.BooleanVar,
                settings_vars["file_operations"]["create_backup_before_move"],
            ),
        ).grid(row=1, column=1, sticky=tk.W, padx=5)
        ...
        ttk.Checkbutton(
            file_ops_frame,
            variable=cast(
                tk.BooleanVar, settings_vars["file_operations"]["skip_hidden_files"]
            ),
        ).grid(row=2, column=1, sticky=tk.W, padx=5)
        ...
        def on_confirm() -> None:
            updated_settings = {
                "llm": {
                    "api_key": cast(
                        tk.StringVar, settings_vars["llm"]["api_key"]
                    ).get(),
                    "model_name": cast(
                        tk.StringVar, settings_vars["llm"]["model_name"]
                    ).get(),
                    "temperature": cast(
                        tk.DoubleVar, settings_vars["llm"]["temperature"]
                    ).get(),
                },
                "ui": {
                    "theme": cast(tk.StringVar, settings_vars["ui"]["theme"]).get(),
                    "language": cast(
                        tk.StringVar, settings_vars["ui"]["language"]
                    ).get(),
                    "show_notifications": cast(
                        tk.BooleanVar, settings_vars["ui"]["show_notifications"]
                    ).get(),
                },
                "file_operations": {
                    "max_history": cast(
                    ...
                    "create_backup_before_move": cast(
                        tk.BooleanVar,
                        settings_vars["file_operations"]["create_backup_before_move"],
                    ).get(),
                    "skip_hidden_files": cast(
                        tk.BooleanVar,
                        settings_vars["file_operations"]["skip_hidden_files"],
                    ).get(),
                },
            }
    ...
    def display_path_suggestions(
        self, file_path: str, suggestions: List[Union[str, PathSuggestion]]
    ) -> None:
        """
    ...
    def display_action_plan(self, plan: List[Dict[str, Any]]) -> None:
        """
        복잡한 작업의 단계들을 보여줍니다.
    ...
    def update_watched_directories(self, directories: List[str]) -> None:
        """
    ...
    def _add_watch_directory(self) -> None:
    ...
    def _remove_watch_directory(self) -> None:
