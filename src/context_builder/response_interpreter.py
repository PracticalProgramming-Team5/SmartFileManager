...
class PathSuggestion(TypedDict):
    """경로 제안 데이터 구조"""
    path: str
    reason: str
...
class ResponseInterpreter:
    ...
    def parse_move_suggestions(self, llm_response: str) -> List[PathSuggestion]:
        """
    ...
    def parse_action_plan(self, llm_response: str) -> List[ActionPlan]:
        """
        ...
        try:
            ...
            if matches:
                ...
                if "plan" in parsed_data and isinstance(parsed_data["plan"], list):
                    action_plan: List[ActionPlan] = cast(List[ActionPlan], parsed_data["plan"])
                    explanation: str = parsed_data.get("explanation", "")
            ...
            if matches:
                ...
                if "plan" in parsed_data and isinstance(parsed_data["plan"], list):
                    return self._validate_action_plan(cast(List[ActionPlan], parsed_data["plan"]))
    ...
    def _validate_action_plan(self, plan: List[ActionPlan]) -> List[ActionPlan]:
        """
