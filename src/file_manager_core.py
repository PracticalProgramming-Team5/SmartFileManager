...
class FileManagerCore:
    ...
    def __init__(self, settings_path):
        ...
        self.context_builder = ContextBuilder(self.filesystem_manager)

        # LLM 클라이언트 초기화
        api_key = self.settings.get("llm", {}).get("api_key", "")
        model_name = self.settings.get("llm", {}).get("model_name", "gpt-4")
        ...
        # 작업 기록 관리자 초기화
        max_history = self.settings.get("file_operations", {}).get("max_history", 20)
    ...
    def start(self):
        ...
        # 감시할 디렉토리 설정
        watched_dirs = self.settings.get("watched_directories", [])
    ...
    def handle_new_file(self, file_path: str):
        ...
        try:
            # 파일 컨텍스트 가져오기
            file_context = self.context_builder.get_file_context(file_path, "partial")

            # 디렉토리 구조 가져오기
            parent_dir = os.path.dirname(file_path)
            dir_structure = self.context_builder.get_directory_structure(parent_dir)

            # LLM에 프롬프트 생성
            prompt = self.context_builder.format_move_prompt(
            ...
            # 응답 파싱하여 제안된 경로 추출
            suggestions = self.response_interpreter.parse_move_suggestions(llm_response)
    ...
    def handle_natural_language_command(self, command: str):
        ...
        try:
            # 디렉토리 구조 가져오기
            watch_dirs = self.settings.get("watched_directories", [])
            dir_structure = ""
            ...
            # 응답 파싱하여 작업 계획 추출
            action_plan = self.response_interpreter.parse_action_plan(llm_response)
    ...
    def execute_file_operation(self, operation: dict):
        ...
        try:
            ...
            if "action" in operation:
                ...
                if action_type == "move":
                    ...
                    result = self.filesystem_manager.rename_item(
                        operation["path"], operation["new_name"]
                    )

                elif action_type == "delete":
                    result = self.filesystem_manager.delete_item(operation["path"])

                elif action_type == "create_directory":
                    result = self.filesystem_manager.create_directory(operation["path"])
            ...
            # 여러 작업 처리 (plan 키가 있는 경우)
                ...
                result = self.filesystem_manager.execute_plan(operation["plan"])

                # 작업이 성공하면 기록에 모든 작업 추가
                if result:
                    for action in operation["plan"]:
            ...
            error_msg = f"파일 작업 실행 중 오류 발생: {str(e)}"
            print(error_msg)
            return False
    ...
    def undo_last_operation(self):
        ...
        # 작업 되돌리기
        result = self.filesystem_manager.reverse_action(last_action)
    ...
    def update_settings(self, new_settings: Dict[str, Any]):
        ...
        # LLM 클라이언트 설정 업데이트
        llm_settings = new_settings.get("llm", {})
        ...
        # 디렉토리 모니터 설정 업데이트
        skip_hidden = new_settings.get("file_operations", {}).get(
            "skip_hidden_files", True
        )
        self.directory_monitor.set_skip_hidden_files(skip_hidden)

        # 작업 기록 관리자 설정 업데이트
        max_history = new_settings.get("file_operations", {}).get("max_history", 20)
